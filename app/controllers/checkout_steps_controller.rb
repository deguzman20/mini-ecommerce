...
class CheckoutStepsController < ApplicationController
  ...
  def show
    ...
    @cart_product.where(cart_id: cart.id).each.each do |cart_product|
  ...
  def calculate(city_result)
    ...
    total_weight = 0
    cart = Cart.find_by_customer_id($customer_id)
    CartProduct.where(cart_id: cart.id).each.each do |cart_product|
    ...
    when 'TAG'
      ...
      when total_weight >= 10_000 then additional = 259.36
      end
    when 'TALIBON'
      case total_weight
      ...
      when total_weight >= 10_000 then additional = 363.04
      end
    when 'ZAM'
      case total_weight
      when 0..5 then additional = 170.00
      ...
      when total_weight >= 10_000 then additional = 311.97
      end
    when 'SJI'
      case total_weight
      when 0..5 then additional = 220.00
      ...
      when total_weight >= 10_000 then additional = 324.409
      end
    when 'LGP'
      case total_weight
      when 0..5 then additional = 150.00
      ...
      when total_weight >= 10_000 then additional = 247.16
      end
    when 'SORSOGON', 'IRIGA'
      case total_weight
      when 0..5 then additional = 150.00
      ...
      when total_weight >= 10_000 then additional = 277.44
      end
    when 'WNP'
      case total_weight
      when 0..5 then additional = 220.00
